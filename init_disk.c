...
int main()
{
    ...
    //1. 初始化SB     大小：1块
    struct sb *super_blk = malloc(sizeof(struct super_block));//动态内存分配，申请super_blk
    ...
    //初始化inode区
    fseek(fp, FS_BLOCK_SIZE *6 , SEEK_SET);
    ...
    root->st_nlink = 0;
    //杂七杂八的权限没搞
    clock_gettime(CLOCK_REALTIME, root->spec);
